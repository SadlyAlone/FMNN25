...
class inexact_line_search:
    ...
    def __call__(self, x_0, s, alpha_0, wolfe_powell = False):
        """
        ...
        """
        self.func_alpha = lambda alpha: self.func(x + alpha*s)
        self.grad_alpha = lambda alpha: self.grad(x + alpha*s)
        ...
        while not (self.left_cond(wolfe_powell) and self.right_cond()):
            if not self.left_cond(wolfe_powell):
                self.extrapolation()
